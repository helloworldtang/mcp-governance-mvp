...
def test_admin_identity_and_passes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        authz, "get_http_request", lambda: _FakeReq({"X-User": "alice", "X-Role": "admin", "X-Trace-Id": "t1"})
    )
...
def test_viewer_denied_at_execution_point(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(authz, "get_http_request", lambda: _FakeReq({"X-User": "bob", "X-Role": "viewer"}))
...
def test_header_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(authz, "get_http_request", lambda: _FakeReq({"x-user": "alice", "x-role": "admin"}))
